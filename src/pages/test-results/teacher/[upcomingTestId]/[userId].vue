...
const getGivenAnswers = async () => {
    const response = await fillTestRepository.listTeacherGetStudentsAnswer(Number(upcomingTestId), Number(userId))

    givenAnswers.value = response
}

const getTestInfo = async () => {
    const response = await fillTestRepository.getTeacherTestInfo(Number(upcomingTestId), Number(userId))
        ...
        <Card>
            <template #title>
                ...
                <div class="flex flex-row justify-between">
                    <h2>{{ testInfo?.user.firstName }} {{ testInfo?.user.lastName }} válaszai</h2>
